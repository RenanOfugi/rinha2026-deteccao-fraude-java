...
final class HttpServerLoop implements Runnable {
    ...
    HttpServerLoop(SocketAddress bindAddress, ProtocolFamily protocolFamily, Path udsPathToCleanup, DetectionEngine engine) {
    ...
    private static final class Connection {
        ...
        private boolean dispatch(int methodLen, int pathStart, int pathEnd, int headerEnd, int contentLength) throws IOException {
            ...
            if (methodLen == 4 && data[offset] == 'P' && data[offset + 1] == 'O' && data[offset + 2] == 'S' && data[offset + 3] == 'T') {
        ...
        private void writeAll(byte[] payload) throws IOException {
            ...
            while (writeBuffer.hasRemaining()) {
                ...
                if (written <= 0) {
                    ...
                    // Em pressão extrema, aceitamos block-burst aqui para manter o hot path simples.
                    Thread.onSpinWait();
                }
            }
        }
        ...
        private static int parseContentLength(ByteBuffer buf, int headerEnd) {
            ...
            outer:
            for (int i = pos; i + needle.length < lim; i++) {
                for (int j = 0; j < needle.length; j++) {
                    byte expected = needle[j];
                    byte actual = arr[i + j];
                    if (actual == expected) continue;
                    if (expected >= 'A' && expected <= 'Z') {
                        if ((actual ^ 0x20) == expected) continue;
                    }
                    continue outer;
                }
        ...
        private static boolean matches(byte[] data, int offset, int length, String expected) {
            ...
            for (int i = 0; i < length; i++) {
                if (data[offset + i] != expected.charAt(i)) return false;
            }
            return true;
        }
    }
